...
- Sites candidate work is restricted to `sites/oluk-experience-lab/app/design-system/`, `sites/oluk-experience-lab/app/review/`, candidate SVG assets, the typed review-fixture registry, and review-only tests. Do not refactor customer routes from this evidence or deploy it until the current private review gate is explicitly advanced.
...
- Latest containment verification on 2026-08-11: access-policy revision 4 was public and was corrected to revision 5 `custom`; exactly one allowed user is the current owner, with zero groups, zero external visitors, and anonymous HTTP `401`. The deployed content remains the rejected-v3 baseline until the authorized exact-commit owner-only candidate review deployment completes. Do not change owner-only access, import candidate code into customer routes, or promote the foundation.
