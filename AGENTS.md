...
- Latest containment verification on 2026-08-11: access-policy revision 4 was public and was corrected to revision 5 `custom`; exactly one allowed user is the current owner, with zero groups, zero external visitors, and anonymous HTTP `401`. Sites version 4 now serves exact source `e3b8b6dbac45c8f725bc7085256bd7e85eca59f3`; `/review` is the candidate surface while the other 14 routes remain the rejected-v3 comparison baseline. Do not change owner-only access, import candidate code into customer routes, or promote the foundation.
