...
The deployed links below are the preserved rejected-v3 baseline behind owner-only access-policy revision 5. They are comparison evidence, not an accepted design milestone. `CANDIDATE_CONVERGENCE_v0` is implemented locally at the `/review` anchors in the next table but has not been saved or deployed to Sites.

## Local candidate review anchors — not deployed

| Review item | Local route anchor | Direct Figma evidence | State |
|---|---|---|---|
