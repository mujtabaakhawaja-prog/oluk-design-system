...
export function RecommendationCard({ state = "default", name = "MK-2866", alias = "Ostarine" }: { state?: RecommendationState; name?: string; alias?: string }) {
  ...
  return <article className={styles.recommendation} data-candidate-component="RecommendationCard" data-state={state}><EvidenceStatusChip state={disabled ? "unavailable" : "source-only"}/><div><span className="eyebrow">SARM SERIES</span><h3>{name}</h3><p>{alias}</p></div><dl className={styles.facts}><div><dt>Strength</dt><dd>15 MG</dd></div><div><dt>Servings</dt><dd>90 SERVINGS</dd></div><div><dt>Price</dt><dd>£43</dd></div></dl><button className={styles.action} disabled={disabled} type="button">{state === "added" ? "Added ✓" : disabled ? "Unavailable" : "Add to bag"}</button></article>;
}
