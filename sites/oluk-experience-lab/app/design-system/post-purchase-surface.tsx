...
export function PostPurchaseSurface({ status, orderId }: { status: PostPurchaseStatus; orderId?: string }) {
  ...
  return <section className="post-purchase-surface"><header className="page-hero"><div className="shell"><span className="eyebrow">{state.eyebrow}</span><h1>{state.title}</h1><p>{state.copy}</p></div></header><div className="shell post-purchase-grid"><section className="post-purchase-main"><div className="post-purchase-reference"><span>ORDER REFERENCE</span><strong>{orderId ?? "OL-10428"}</strong></div>{state.amount ? <LifecycleAmountRecord stage="confirmation"/> : null}{status === "tracking" ? <ol className="tracking-timeline"><li data-active="true"><b>01</b><div><strong>Order confirmed</strong><span>Your order has been received.</span></div></li><li data-active="true"><b>02</b><div><strong>Preparing your order</strong><span>Your product is being prepared for dispatch.</span></div></li><li><b>03</b><div><strong>On its way</strong><span>Tracking details appear here when ready.</span></div></li><li><b>04</b><div><strong>Delivered</strong><span>The final delivery update will appear here.</span></div></li></ol> : <div className="post-purchase-actions"><Link className="button" href={state.href}>{state.action} →</Link><Link href="/account/orders">View all orders →</Link></div>}</section><aside className="post-purchase-side"><RestockCard state="active"/><RecommendationCard state="default"/></aside></div></section>;
}
