...
import { getCustomerProductCardFixture } from "./product-content-card-adapter";
...
export const openLabPortalEntries = [
  ...
  { index: "05", title: "Comparison availability", copy: "Compare only when at least two customer-ready product records exist.", href: "/open-lab/compare" },
  { index: "06", title: "Relationship availability", copy: "See whether an approved product relationship is available.", href: "/open-lab/stack-builder" },
] as const;
...
export function OpenLabPortalHero() {
  const product = getCustomerProductCardFixture("mk-2866");
  return <section className={styles.portalHero} data-module="OpenLabPortalHero"><div className={styles.shell}><div className={styles.portalGrid}><EditorialSurface actions={<><ActionLink href="/open-lab/records">Browse lab records</ActionLink><ActionLink href="/open-lab/stack-builder" secondary>Check relationship availability</ActionLink></>} copy="Move from a product into its available batch record and source context, then return to the matching product without borrowing another product’s result." eyebrow="OPENLAB" headingLevel="h1" title="Product confidence, made visible."><div className={styles.lenses} role="list" aria-label="OpenLab customer value"><span role="listitem">PRODUCT CONFIDENCE</span><span role="listitem">SOURCE ACCESS</span><span role="listitem">CONNECTED COMMERCE</span></div></EditorialSurface>{product ? <ProductCommerceCard className={styles.portalProduct} headingLevel="h2" product={product} showQualitative={false} variant="vertical"/> : <PresentationState state="unavailable" title="Featured product information is unavailable."/>}</div></div></section>;
}

export function OpenLabWayfinding() {
  return <section className={styles.section} data-module="OpenLabWayfinding"><div className={styles.shell}><DecisionSurface copy="Enter through a product, exact reference or source explanation. Comparison and relationship guidance remain explicit when their content is unavailable." eyebrow="CHOOSE THE WAY IN" title="Start with the question you want OpenLab to answer."><div className={styles.wayfinding}>{openLabPortalEntries.map((entry)=><a href={entry.href} key={entry.title}><span>{entry.index}</span><h3>{entry.title}</h3><p>{entry.copy}</p><Arrow/></a>)}</div></DecisionSurface></div></section>;
}
...
export function OpenLabStateLegend() {
  return <section className={styles.legend} data-module="OpenLabStateLegend" aria-label="Evidence state legend">{(["source-reported","source-only","unavailable"] as const).map((state)=><EvidenceStatusChip key={state} state={state}/>)}</section>;
}
...
export function OpenLabSourceChain() {
  return <section className={styles.section} data-module="OpenLabSourceChain"><div className={styles.shell}><TechnicalSurface actions={<ActionLink href="/open-lab/records">Find a record</ActionLink>} copy="The chain keeps the exact product, batch and source connected all the way back to the matching product decision." eyebrow="SOURCE CHAIN" title="See how product confidence is assembled."><div className={styles.chain}>{sourceChainStages.map(([index,title,copy])=><article key={title}><span>{index}</span><div><h2>{title}</h2><p>{copy}</p></div><Chevron/></article>)}</div></TechnicalSurface></div></section>;
}

export function OpenLabComparison() {
  return <section className={styles.section} data-module="OpenLabComparison"><div className={styles.shell}><PresentationState action={<ActionLink href="/product/mk-2866">View the available MK-2866 record</ActionLink>} copy="A comparison requires at least two customer-ready product records. Incomplete facts and MK-2866 evidence are not promoted or copied into another product column." eyebrow="COMPARISON" state="unavailable" title="Product comparison is not available yet."/></div></section>;
}
