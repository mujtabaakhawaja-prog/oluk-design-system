...
const compounds = [
  ...
  { name: "MK-677", alias: "Ibutamoren", strength: "10 MG", family: "GH SERIES", status: "source-only" as const },
...
function StackBuilder() {
  const goals=["Build","Cut","Recomp","Recovery"];
  return <div className={styles.page}><Header eyebrow="OPENLAB · STACK BUILDER" title="Build a clearer product combination." copy="Choose the outcome, compare the roles, then keep every selected product connected to its own specifications and evidence record."/><ToolNav active="stack-builder"/><section className={styles.builder}><div className={styles.builderSteps}>{["Choose a goal","Select a foundation","Add a complement"].map((step,index)=><div key={step}><b>{index+1}</b><span>{step}</span></div>)}</div><div className={styles.goalPicker}>{goals.map((goal,index)=><button className={index===1?styles.selected:undefined} key={goal} type="button"><span>0{index+1}</span>{goal}</button>)}</div><div className={styles.stackResult}><div><span>SELECTED PATH · CUT</span><h2>Ostarine + Cardarine</h2><p>A compact two-product composition with separate product, strength and evidence paths preserved for each selection.</p><div className={styles.stackProducts}><article><b>01</b><span>SARM SERIES</span><h3>MK-2866</h3><p>15 MG · 90 SERVINGS</p></article><i>+</i><article><b>02</b><span>METABOLIC SERIES</span><h3>GW-501516</h3><p>10 MG · 90 SERVINGS</p></article></div></div><aside><span>YOUR BUILD</span><dl><div><dt>Products</dt><dd>2</dd></div><div><dt>Goal</dt><dd>Cut</dd></div><div><dt>Duration view</dt><dd>8 weeks</dd></div></dl><button type="button">View complete stack</button></aside></div></section></div>;
}

function Calculator() {
  return <Workspace active="dosing-calculator" eyebrow="OPENLAB · DOSE CALCULATOR" title="Translate a product format into a clear schedule." copy="Turn strength, frequency and duration into an easy-to-read product schedule."><section className={styles.calcGrid}><article className={styles.formCard}><span>CALCULATION INPUT</span><h2>Plan the schedule</h2><label>Product<select defaultValue="mk"><option value="mk">MK-2866 · 15 MG</option><option>RAD-140 · 8 MG</option><option>MK-677 · 10 MG</option></select></label><label>Daily target<div className={styles.numberInput}><input defaultValue="15" inputMode="decimal"/><b>MG</b></div></label><label>Plan length<div className={styles.numberInput}><input defaultValue="8" inputMode="numeric"/><b>WEEKS</b></div></label><button type="button">Update schedule</button></article><article className={styles.resultCard}><span>8-WEEK VIEW</span><strong>56</strong><h2>scheduled days</h2><div className={styles.weekGrid}>{Array.from({length:8},(_,i)=><div key={i}><b>W{i+1}</b><span>{i<6?"15 MG daily":"Review"}</span></div>)}</div><div data-card-footer><span>Product total</span><b>840 MG</b></div></article></section></Workspace>;
}
