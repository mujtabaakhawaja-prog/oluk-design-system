...
export function PaymentTrustPrimer({ compact = false }: { compact?: boolean }) {
  return <section aria-label="Payment trust" className={styles.primer} data-compact={compact || undefined} data-copy-surface="transaction" data-live-authority="false">
...
export function LifecycleAmountRecord({ stage }: { stage: Exclude<PaymentTrustLifecycleStage, "pre-purchase" | "payment"> }) {
  ...
  return <section aria-label={heading} className={styles.record} data-copy-surface="transaction" data-stage={stage} data-live-authority="false">
    <span>{heading}</span>
