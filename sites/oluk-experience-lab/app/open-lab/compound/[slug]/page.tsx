...
export default function CompoundDossierPage({params}:{params:{slug:string}}){const product=getFrontierProduct(params.slug);if(!product)return notFound();return <CustomerSiteChrome route="openlab"><main data-compound={product.slug}><h1 className="sr-only">{product.name} / {product.alias}</h1><OpenLabDossierComposition productSlug={product.slug}/></main></CustomerSiteChrome>}
