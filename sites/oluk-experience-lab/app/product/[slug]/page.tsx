...
export default function FrontierProductPage({params}:{params:{slug:string}}){const product=getFrontierProduct(params.slug);if(!product)return notFound();return <CustomerSiteChrome route="product"><main><script dangerouslySetInnerHTML={{__html:JSON.stringify(productJsonLd(product))}} type="application/ld+json"/><ProductHero product={product}/><ProductNarrative product={product}/><ProductContinuation product={product}/></main></CustomerSiteChrome>;}
