...
test("OpenLab core is a declared canonical module system",async()=>{const registry=JSON.parse(await readFile(path.join(repoRoot,"authority/OPENLAB-SECTION-MODULE-REGISTRY.json"),"utf8"));assert.equal(registry.runtimeAuthority,"NONE");assert.equal(registry.canonicalNamespace,"/open-lab");assert.equal(registry.modules.length,10);assert.deepEqual(registry.ownerOnlyRoutes,["/open-lab/admin"]);for(const entry of registry.modules){assert.ok(entry.export);assert.ok(entry.dataOwner);assert.ok(entry.mounts.length);assert.ok(entry.mobileStrategy);assert.ok(entry.runtimeExclusions.length);}});
