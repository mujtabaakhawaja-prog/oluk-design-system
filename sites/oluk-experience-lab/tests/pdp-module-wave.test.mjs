...
test("PDP route mounts canonical sections and preserves approved content",async()=>{const [route,sections,firstFold]=await Promise.all([readSite("app/customer-routes.tsx"),readSite("app/design-system/pdp-sections.tsx"),readSite("app/design-system/pdp-first-fold.tsx")]);for(const component of ["PdpFirstFold","ProductDetailDisclosure","ProductEvidenceSnapshot","MobileDecisionSummary"])assert.match(route,new RegExp(`<${component}`));assert.match(firstFold,/data-figma-node="1155:30632"/);assert.match(firstFold,/<PurchasePanel/);assert.match(sections,/Third-Party Tested\./);assert.match(sections,/15 MG|product\.strength/);assert.match(sections,/90 SERVINGS|product\.servings/);assert.match(sections,/product\.purity/);assert.doesNotMatch(`${sections}\n${firstFold}`,/fetch\(|XMLHttpRequest|WebSocket|localStorage|sessionStorage/);});

test("every frontier PDP uses the PdpD purchase anatomy and never invents missing product media",async()=>{const [route,composition,presentation,chamber]=await Promise.all([readSite("app/product/[slug]/page.tsx"),readSite("app/product/product-page.tsx"),readSite("app/design-system/frontier-product-presentation.ts"),readSite("app/design-system/product-media-chamber.tsx")]);assert.match(route,/ProductPageComposition product=\{product\}/);for(const component of ["PdpFirstFold","AssuranceRail","ProductDetailDisclosure","ProductDossier","ProductEvidenceSnapshot","ProductContinuation","MobileDecisionSummary"])assert.match(composition,new RegExp(`<${component}`));assert.match(composition,/frontierProductPresentation/);assert.match(presentation,/if \(!asset \|\| !crops\) return null/);assert.match(chamber,/unpopulated-governed-chamber/);});
